-- Panel moderacji: lista adminow, flaga bana i status zgloszen.
-- Uprawnienia moderacyjne NIE ida przez RLS klienta — wszystkie akcje wykonuje
-- edge function admin-actions (service role) po zweryfikowaniu, ze wolajacy
-- jest w admin_users. Klient moze jedynie sprawdzic, czy SAM jest adminem.
...
drop policy if exists "check own admin" on admin_users;
create policy "check own admin" on admin_users for select using (auth.uid() = user_id);

alter table profiles add column if not exists banned boolean not null default false;

alter table reported_users add column if not exists status text not null default 'pending';
alter table reported_users add column if not exists resolved_at timestamptz;

-- Zbanowani znikaja z talii swipe
