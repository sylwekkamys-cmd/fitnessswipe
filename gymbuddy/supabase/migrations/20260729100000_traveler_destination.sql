-- Rozbudowa trybu "Jestem tu przejazdem": dotychczas znacznik traveler_until
-- byl tylko odliczaniem dni bez informacji GDZIE ta osoba jest — a talia swipe
-- i tak liczyla dystans od lokalizacji DOMOWEJ (profiles.latitude/longitude),
-- wiec podrozny nie byl realnie widoczny osobom w miejscu docelowym.
-- Dodajemy: nazwe miejscowosci, wspolrzedne celu podrozy i krotka notatke.
alter table profiles add column if not exists traveler_city text;
alter table profiles add column if not exists traveler_lat double precision;
alter table profiles add column if not exists traveler_lng double precision;
alter table profiles add column if not exists traveler_note text;

-- Postgres nie pozwala CREATE OR REPLACE zmienic zestawu kolumn wyjsciowych
-- (dodajemy traveler_city/traveler_note) — trzeba najpierw usunac funkcje.
...
AS $function$
  select
    ...
    -- Aktywny podroznik: dystans i pozycja licza sie od miejsca docelowego, nie domu
