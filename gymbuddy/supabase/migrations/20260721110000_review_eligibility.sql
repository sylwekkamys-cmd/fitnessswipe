-- Zaostrzenie prawa do oceny trenera: sam match juz nie wystarcza
-- (kazdy moze napisac do trenera, co tworzy match — to bylo za slabe).
-- Teraz: wspolny trening w dzienniku (trener jako partner, w dowolna strone)
-- LUB udzial w wydarzeniu UTWORZONYM przez tego trenera.
...
begin
  select exists(
    ...
    where (w.creator_id = p_viewer_id and w.partner_id = p_trainer_id)
       or (w.creator_id = p_trainer_id and w.partner_id = p_viewer_id)
  ) or exists(
    select 1 from sports_events se
    join event_attendees ea on ea.event_id = se.id
    where se.creator_id = p_trainer_id and ea.profile_id = p_viewer_id
  ) into v_can_review;
...
begin
  ...
  select exists(
    select 1 from workouts w
    where (w.creator_id = p_reviewer_id and w.partner_id = p_trainer_id)
       or (w.creator_id = p_trainer_id and w.partner_id = p_reviewer_id)
  ) or exists(
    select 1 from sports_events se
    join event_attendees ea on ea.event_id = se.id
    where se.creator_id = p_trainer_id and ea.profile_id = p_reviewer_id
  ) into v_can_review;
